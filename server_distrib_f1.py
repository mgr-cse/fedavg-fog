...
parser.add_argument('-srf', '--server_fraction', type=float, default=0.4, help='fraction of other servers selected')
parser.add_argument('-of', '--obeserve_file', type=str, default='test_run_distrib_f1', help='file for obeservations')
...
if __name__=='__main__':
    ...
    class servers:
        ...
        # similarity
        def getf1(self, server):
            for variable, value in zip(tf.trainable_variables(), self.serverVars[server]):
            ...
            return f1_score(y_true_run, y_pred_run, average=None)
        
        def getsim(self, server1, server2):
            f1 = self.getf1(server1)
            f2 = self.getf1(server2)
        ...
        for i in tqdm(range(args.num_comm)):
            ...
            if i % args.val_freq == 0:
                ...
                for j in range(myServers.num):
                    ...
                    similarities = {}
                    for s in serv_set:
                        similarities[s] = myServers.getsim(s, f'server{j}')
                    
                    sorted_tup = sorted(similarities.items(), key=lambda kv: (kv[1], kv[0]))
                    print(sorted_tup)
                    
                    # select dissimilar servers
